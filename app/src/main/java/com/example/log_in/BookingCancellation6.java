...
public class BookingCancellation6 extends AppCompatActivity {
    ...
    private String selectedOption6; // Declare this variable at the class level

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_booking_cancellation6);
        // Initializing views
        ...
        spinRefund.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> parentView, View selectedItemView, int position, long id) {
                ...
                selectedOption6 = parentView.getItemAtPosition(position).toString();
        ...
        detailsclick.setOnClickListener(v -> {
            Intent intent = new Intent(BookingCancellation6.this, BookingDetailMain.class);
        ...
        dialog = new Dialog(BookingCancellation6.this);
...
// Buttons for withdrawal dialog
        ...
        notnowbtn.setOnClickListener(v -> {
            Intent backIntent = new Intent(BookingCancellation6.this, BookingDetailMain6.class);
            startActivity(backIntent);
            Toast.makeText(BookingCancellation6.this, "Not now", Toast.LENGTH_SHORT).show();
            dialog.dismiss();
        });

// CONFIRMATION BUTTON, NAVIGATE TO THE BOOKING DETAIL MAIN
        confirmbtn.setOnClickListener(v -> {
            Intent confirmIntent = new Intent(BookingCancellation6.this, BookingDetailMain6.class);
...
// Initialization and actions for continue dialog
        ...
        btnNotnow.setOnClickListener(v -> {
            Intent notnowIntent = new Intent(BookingCancellation6.this, BookingDetailMain6.class);
        ...
        btnConfirm.setOnClickListener(v -> {
            ...
            Toast.makeText(BookingCancellation6.this, "Confirm Cancellation, please wait for approval", Toast.LENGTH_SHORT).show();
            dialog.dismiss();
        });
    ...
    private void saveSelectedOptionToFirestore() {
        ...
        if (currentUser != null) {
            userDocRef
                    .update("selectedRefundOption6", selectedOption6)
                    .addOnSuccessListener(aVoid -> {
                        // Show a toast message indicating that the option has been saved successfully
                        Toast.makeText(BookingCancellation6.this, "Selected refund option saved successfully", Toast.LENGTH_SHORT).show();
                    })
                    .addOnFailureListener(e -> {
                        // Show a toast message indicating an error in saving the option
                        Toast.makeText(BookingCancellation6.this, "Error saving refund option: " + e.getMessage(), Toast.LENGTH_SHORT).show();
                    });
        }
    }
    ...
    private void retrieveDataFromFirestore() {
        ...
        userDataListener = db.collection("users")
                ...
                .addSnapshotListener((documentSnapshot, error) -> {
                    ...
                    if (error != null) {
                        Log.e("BookingCancellation6", "Error fetching user data: " + error.getMessage());
                        return;
                    }
                    if (documentSnapshot != null && documentSnapshot.exists()) {
                        String selectedTour6 = documentSnapshot.getString("selectedTour6");
                        String E_mail = documentSnapshot.getString("Email");
                        String selectedOption6 = documentSnapshot.getString("selectedRefundOption6");
                        double total6 = documentSnapshot.getDouble("totalAmount6");
                        // Use getDouble for numeric values
                        String selectedTouristNum6= documentSnapshot.getString("selectedTouristNum6");

                        // Check if the views are not null before setting values
                        if (TotalTouristsText != null) {
                            TotalTouristsText.setText(selectedTouristNum6);
                        }
                        if (amountText != null) {
                            amountText.setText(String.format("₱%.2f", total6));
                        }

                        if (pickhouseText != null) {
                            pickhouseText.setText(selectedTour6);
                        }
                        if (nameText != null) {
    ...
    private void saveOptionToFirestore() {
        ...
        if (currentUser != null) {
            userDocRef
                    .update("selectedRefundOption6", selectedOption6)  // Update the selected refund option in Firestore
                    .addOnSuccessListener(aVoid -> {
                        // Show a toast message indicating that the option has been saved successfully
